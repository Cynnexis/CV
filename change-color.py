...
p.add_argument(
	...
	"--primary-text-color",
	default=None,
	help="The text color with the primary color as a background. Default is white (#FFFFFF).")
...
p.add_argument(
	"--h3-color", default=None, help="The header 2 color. Used for bullet and date. Default is grey (#938690).")
...
def change_color(new_color: str, default_old_color: str, color_name: str, update_image: bool):
	if new_color is not None:
		...
		with open("resume.cls", "r+") as f:
			resume_cls = f.read()
			color_matches = re.findall(r"\\definecolor\{" + color_name + r"\}\{HTML\}\{#?([0-9a-fA-F]{6})\}",
										resume_cls, re.MULTILINE)
			if len(color_matches) > 0:
				old_color = color_matches[0]
			# Replace occurrence in resume.cls
			new_resume_cls, nb_chgmt = re.subn(
				r"\\definecolor\{" + color_name + r"\}\{HTML\}\{" + old_color + r'\}',
				r"\\definecolor{" + color_name + "}{HTML}{" + re.sub("(#|0x)", '', new_color) + "}", resume_cls,
				re.IGNORECASE)
			if nb_chgmt > 0:
				f.seek(0)
		...
		if update_image:
			...
			for image_path in images_path:
				with open(image_path, 'r+') as f:
					...
					new_image_content, nb_chgmt = re.subn(
						'#' + old_color, new_color, image_content, flags=re.IGNORECASE)
					if nb_chgmt > 0:
						f.seek(0)
						...
						# Call Inkscape
						subprocess.call([
							INKSCAPE_PATH, "-z", image_path, "-e",
							image_path.replace(".svg", ".png"), "-C", "-w",
							str(OUTPUT_IMAGES_SIZE), "-h",
							str(OUTPUT_IMAGES_SIZE)
						])
